...
public class FileHelper {
	...
	public static String CSS_FILE = "diffToHtml.css";
	...
	public static void copyCssFileToDir(String dirPath) throws IOException {
		...
		Files.copy(Paths.get(cssFileSourcePath), Paths.get(cssFileDestPath), StandardCopyOption.REPLACE_EXISTING);
	}
	...
	public static List<String> readAllLinesWithDetectedEncoding(String filePath) throws IOException {
		String encoding = FileHelper.getCharsetEncoding(filePath);
		if (encoding == null) {
			return readAllLines(filePath, Charsets.US_ASCII);

		} else if (encoding == Constants.CHARSET_UTF_8) {
			return readAllLines(filePath, Charsets.UTF_8);

		} else if (encoding == Constants.CHARSET_WINDOWS_1252) {
			return readAllLines(filePath, Charsets.ISO_8859_1);

		} else if (encoding == Constants.CHARSET_UTF_16BE) {
			return readAllLines(filePath, Charsets.UTF_16BE);

		} else if (encoding == Constants.CHARSET_UTF_16LE) {
			return readAllLines(filePath, Charsets.UTF_16LE);
